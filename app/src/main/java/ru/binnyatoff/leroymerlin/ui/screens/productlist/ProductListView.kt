...
fun ProductListView(viewModel: ProductListViewModel, navController: NavController) {
    ...
    when (val state = viewState.value) {
        is ProductListState.Loaded -> ProductListLoaded(
...
fun ProductListLoaded(
    ...
    Scaffold(topBar = {
        ...
        LazyColumn(
            ...
            items(productList, key = { product ->
                product.productId
            }) { product ->
                ProductView(product = product,
                    onClick = {
                        navController.navigate(
                            "${NavigationTree.ProductDetails.name}/" + product.productId
                        )
                    },
                    onClickBag = {
                        viewModel.obtainAction(ProductListAction.ClickedBag(product))
                    },
                    onClickShopList = {
                        viewModel.obtainAction(
                            ProductListAction.ClickedShopList(
                                product.productId,
                                !product.inShopList
                            )
                        )
